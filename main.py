...
@app.get("/generate-week-preview")
def generate_week_preview_endpoint(athlete_id: int, week_start: str, db: Session = Depends(get_db)):
    try:
        result = generate_week_preview(db, athlete_id, week_start)
        return result
    except Exception as e:
        return {
            "status": "error",
